...
public final class TextDrawingArea extends DrawingArea {
	...
	public void drawCenteredText(int colour, int x, String text, int y, boolean shadow) {
		drawRegularText(shadow, x - getTextWidth(text) / 2, colour, text, y);
	}
	...
	public int getTextWidth(String text) {
		...
		for (int charIndex = 0; charIndex < text.length(); charIndex++) {
			if (text.substring(charIndex).startsWith("<img=") && text.substring(charIndex).contains(">")
					&& !(text.substring(charIndex + 5).contains("<img=")
					&& text.substring(charIndex + 5).indexOf("<img=") < text.substring(charIndex + 5).indexOf(">") )) {
				String rest = text.substring(charIndex + 5);
				Sprite sprite = SpriteLoader.sprites[Integer.parseInt(rest.substring(0, rest.indexOf(">")))];
				j += sprite.myWidth + 1;
				charIndex += charIndex + 5 + text.substring(charIndex + 5).indexOf(">") - charIndex;
			} else if (text.charAt(charIndex) == '@' && charIndex + 4 < text.length() && text.charAt(charIndex + 4) == '@')
				charIndex += 4;
			else {
				j += rsb[text.charAt(charIndex)];
			}
		}
		return j;
	}
	...
	public void drawRegularText(boolean shadow, int xPos, int color, String text, int yPos) {
		aBoolean1499 = false;
		int l = xPos;
		if (text == null)
			return;
		...
		for (int charIndex = 0; charIndex < text.length(); charIndex++) {
			if (text.substring(charIndex).startsWith("<img=") && text.substring(charIndex).contains(">")
				...
				String rest = text.substring(charIndex + 5);
				Sprite sprite = SpriteLoader.sprites[Integer.parseInt(rest.substring(0, rest.indexOf(">")))];
				sprite.drawSprite(xPos, yPos + 1);
				xPos += sprite.myWidth + 1;
				charIndex += charIndex + 5 + text.substring(charIndex + 5).indexOf(">") - charIndex;
			} else if (text.charAt(charIndex) == '@' && charIndex + 4 < text.length() && text.charAt(charIndex + 4) == '@') {
				int j1 = getColorByName(text.substring(charIndex + 1, charIndex + 4));
				if (j1 != -1)
					color = j1;
				charIndex += 4;
			} else {
				...
				if (c != ' ') {
					...
					method392(aByteArrayArray1491[c], xPos + anIntArray1494[c], yPos + anIntArray1495[c], anIntArray1492[c], anIntArray1493[c], color);
				}
				xPos += rsb[c];
			}
		}
		if (aBoolean1499)
			DrawingArea.drawLine(yPos + (int) ((double) anInt1497 * 0.69999999999999996D), 0x800000, xPos - l, l);
	}
