...
final class ObjectOnTile extends Animable {
	...
	private ObjectDefinition getObjectForVarbitConfig()
	{
		...
		if(varbitIndex != -1)
		{
			...
			if(objectId == 7849) {
                //System.out.println(objectId + ", config=" + k + ", least=" + l + ",most=" + i1 + ", value=" + i+", "+objectConfigs[(int)i]+", "+clientInstance.variousSettings[k]+", "+j1);
            }
			//System.out.println(objectId+", config="+k+", least="+l+",most="+i1+", value="+i);

		} else
            if(configId != -1)
                i = clientInstance.variousSettings[configId];

		if(i < 0 || i >= objectConfigs.length || objectConfigs[(int) i] == -1)
			return null;
		else {
            return ObjectDefinition.forID(objectConfigs[(int) i]);
        }
